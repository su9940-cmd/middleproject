...
def test_model_failure_persists_the_reading_but_reports_a_clean_error(monkeypatch, client):
    ...
    def fail_prediction(**kwargs):
        raise ModelArtifactError("test model unavailable")

    monkeypatch.setattr("app.services.ml_service.predict_risk_score", fail_prediction)
...
def test_existing_active_alert_is_reused_not_reminted(monkeypatch, client):
    ...
    assert body["alert_status"] == AlertStatus.MONITORING
