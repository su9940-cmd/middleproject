...
def test_worker_interrupt_resumes_with_the_submitted_response():
    ...
    normalized = result["worker_response"]
    assert normalized["response_id"] == worker_response["response_id"]
    assert normalized["alert_id"] == worker_response["alert_id"]
    assert normalized["checklist_id"] == "CL-AL-M0101-resume-case-V1"
    assert normalized["item_results"][0]["checklist_item_id"] == "CI-1"
