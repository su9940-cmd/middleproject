...
def test_call_returns_only_memory_context_key():
...
def test_empty_history_returns_all_contract_fields():
    agent = _make_agent()
...
def test_filters_out_other_machines():
    alerts = [
    ...
    ctx = agent({"machine_id": MACHINE, "risk_level": RiskLevel.CAUTION})["memory_context"]
    assert ctx["previous_alert_count"] == 1


def test_maintenance_history_is_forwarded_intact():
    maintenance = [
        {
    ...
    assert ctx["latest_worker_note"] == "냉각수 부족 의심"
    assert ctx["maintenance_history"] == maintenance
...
def test_repeat_limit_triggered_by_state():
    agent = _make_agent()
    ctx = agent(
        {"machine_id": MACHINE, "risk_level": RiskLevel.WARNING, "repeat_count": REPEAT_LIMIT_THRESHOLD}
    )["memory_context"]
    assert ctx["is_repeat_limit_exceeded"] is True


def test_repeat_limit_triggered_by_unresolved_count():
...
def test_missing_machine_id_raises_memory_lookup_error():
    agent = _make_agent()
    with pytest.raises(MemoryLookupError) as exc_info:
        agent({"risk_level": RiskLevel.CAUTION})

    assert exc_info.value.error_code == "MEMORY_LOOKUP_FAILED"
    assert exc_info.value.details["failed_node"] == "memory_agent"


def test_repository_exception_is_wrapped_as_memory_lookup_error():
    ...
    assert exc_info.value.error_code == "MEMORY_LOOKUP_FAILED"
    assert exc_info.value.details["failed_node"] == "memory_agent"
    assert exc_info.value.details["machine_id"] == MACHINE
    # 원본 예외가 __cause__로 보존되어야 함
    assert isinstance(exc_info.value.__cause__, RuntimeError)


def test_agent_does_not_return_error_dict_on_failure():
    ...
    pytest.fail(f"예외를 raise해야 하는데 dict를 반환함: {result}")
