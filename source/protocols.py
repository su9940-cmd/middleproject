#1. app/repositories/protocols.py — 리포지토리 인터페이스 (Protocol)

# Memory Agent가 필요로 하는 DB 조회 함수의 시그니처만 정의
# C가 나중에 실제 구현체를 만들 때 이 프로토콜을 참고
# 왜 필요한가: B가 C를 기다리지 않고 진행할 수 있게 하는 계약서. Duck typing이라 C의 실제 클래스가 Protocol을 명시적으로 상속하지 않아도 시그니처만 맞으면 동작함
